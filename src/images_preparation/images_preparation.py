...
def create_directory(
    class_name: str, train_dir: str, valid_dir: str, test_dir: str
) -> Dict[str, str]:
...
def create_train_valid_test_directories(
    data_dir: str, classes: List[str]
) -> List[Dict[str, str]]:
...
def create_lists_of_train_and_valid_count_of_classes(
    list_of_cls_names: List[StringList], train_ratio: float, valid_ratio: float
) -> Tuple[NumList, NumList]:
...
def copy_classes_to_directories(
    list_of_cls_names: List[StringList],
    list_of_train_classes: List[int],
    list_of_valid_classes: List[int],
    list_of_classes_dir_dicts: List[Dict[str, str]],
    classes: List[str],
    base_dir: str,
) -> None:
        ...
        for i, file_name in enumerate(cls_names):
            if i <= train_idx_cls:
                copy_files_to_directories(
                    "train", class_name, file_name, class_dir_dict, base_dir
                )
            if train_idx_cls < i <= valid_idx_cls:
                copy_files_to_directories(
                    "valid", class_name, file_name, class_dir_dict, base_dir
                )
            if valid_idx_cls < i <= len(cls_names):
                copy_files_to_directories(
                    "test", class_name, file_name, class_dir_dict, base_dir
                )
    ...
    for class_name, class_dir_dict in zip(classes, list_of_classes_dir_dicts):
        for dataset in ["train", "valid", "test"]:
            ...
            print(
                f'[INFO] Count of images for class "{class_name}" in {dataset} dataset: {count_of_images}'
            )
...
def prepare_images(
    ...
    (
        list_of_train_classes,
        list_of_valid_classes,
    ) = create_lists_of_train_and_valid_count_of_classes(
        list_of_cls_names, train_ratio, valid_ratio
    )
    copy_classes_to_directories(
        list_of_cls_names,
        list_of_train_classes,
        list_of_valid_classes,
        list_of_classes_dir_dicts,
